...
const allowedTiers = ['Easy', 'Medium', 'Brave']
...
export default async function handler(req, res) {
  ...
  const tier = allowedTiers.includes(req.body?.tier) ? req.body.tier : 'Medium'
  ...
  const prompt = `Create a completely original, warm English reading adventure for ages 6-9.
Interest: ${interest}. Reading tier: ${tier}. Naturally reuse these review words: ${reviewWords.join(', ') || 'brave, discover'}.
Write exactly 5 paragraphs, each 55-85 words. Maintain one coherent plot with setup, problem, two attempts, and resolution.
No violence, romance, brands, personal data, external links, frightening danger, or moralizing. Do not include a child's name.
...
Create 5 vocabulary items used in the story, each with a simple definition, 3 choices, and correct index.
